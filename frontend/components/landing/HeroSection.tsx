    ...
    <div className="flex-1 px-4 md:px-6 pt-20 pb-4 md:pb-6 flex items-end">
      <div className="relative w-full rounded-2xl overflow-hidden" style={{ height: "calc(100vh - 96px)" }}>
        ...
        <div className="relative z-10 flex flex-col items-start justify-start h-full p-6 md:p-12 pt-24 md:pt-36">
          {/* Badge */}
          <span className="inline-flex items-center gap-2 bg-white/80 backdrop-blur text-black text-xs font-medium px-3 py-1.5 rounded-full mb-5">
            <span className="w-2 h-2 rounded-full bg-[#4ADE80] animate-pulse" />
            Built on Celo — Mainnet Live
          </span>

          <h1 className="text-black text-4xl md:text-6xl font-medium leading-tight max-w-xl mb-4" style={{ letterSpacing: "-0.04em" }}>
            Trustless Arisan,<br />On-Chain.
          </h1>

          <p className="text-black/70 text-sm md:text-lg max-w-md mb-8 leading-relaxed">
          ...
          <div className="flex flex-wrap gap-3">
            <Link href="/app" className="inline-flex items-center gap-3 bg-[#86EFAC] text-black text-base font-medium pl-7 pr-2 py-2 rounded-full hover:bg-[#4ADE80] transition-colors duration-200">
              Launch App
              <span className="bg-black rounded-full p-2"><ArrowRight className="w-4 h-4 text-[#86EFAC]" /></span>
            </Link>
            <a href="#how" className="inline-flex items-center bg-white/80 backdrop-blur text-black text-base font-medium px-7 py-2 rounded-full hover:bg-white transition-colors duration-200">
              How it works
            </a>
          </div>

          {/* Ecosystem marquee */}
          <div className="mt-16 md:mt-24 w-full max-w-md overflow-hidden">
            <div className="marquee-track">
              {[...ecosystem, ...ecosystem].map((item, i) => (
                <span key={i} className="mx-7 shrink-0 text-black/50 whitespace-nowrap" style={item.style}>{item.name}</span>
              ))}
            </div>
          </div>
        </div>
